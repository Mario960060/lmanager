...
const Dashboard = () => {
  ...
  const DayView = ({ date, dayEvents }: { date: Date; dayEvents: typeof events }) => {
    ...
    // Initialize expanded sections for this day if not exists
    if (!expandedSections[format(date, 'yyyy-MM-dd')]) {
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
                    ...
                    {(eventMaterials.length > 0 || eventEquipment.length > 0) && (
                      <div className="flex items-center text-gray-600">
                        ...
                        <span>
                          {eventMaterials.length > 0 && `${eventMaterials.length} materials`}
                          {eventMaterials.length > 0 && eventEquipment.length > 0 && ', '}
                          {eventEquipment.length > 0 && `${eventEquipment.length} equipment`}
                        </span>
                      </div>
                    )}
                  </div>
                        ...
                        <div className="space-y-2">
                          {eventEquipment.map(equipment => (
                            <div key={equipment.id} className="bg-white p-2 rounded-md text-sm">
                              <div className="flex justify-between">
                                <span className="font-medium">
                                  {equipment.equipment?.name} - {equipment.quantity} {equipment.quantity > 1 ? 'units' : 'unit'}
                                </span>
                                {equipment.notes && (
