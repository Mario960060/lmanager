    ...
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-0 md:p-4">
        <div className="rounded-lg shadow-2xl w-full md:max-w-5xl max-h-[100vh] md:max-h-[90vh] md:rounded-lg flex flex-col overflow-hidden" style={{ backgroundColor: colors.bgCard }}>
          ...
          <div className="border-b px-3 py-3 md:p-4 flex justify-between items-center flex-shrink-0">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <h2 className="text-2xl font-bold" style={{ color: colors.textPrimary }}>{step.title}</h2>
                <PageInfoModal
              ...
              <p className="text-sm ml-11" style={{ color: colors.textMuted }}>{step.description}</p>
            </div>
            <button
              className="p-2 rounded-lg transition-colors"
              onClick={() => setShowExitModal(true)}
            >
              <X className="w-6 h-6" style={{ color: colors.textSubtle }} />
            </button>
          </div>
              ...
              {steps.map((s) => (
                <div key={s.number} className="flex-1 h-1 rounded-full overflow-hidden" style={{ backgroundColor: colors.borderLight }}>
                  <div
                    ...
                    style={{ backgroundColor: s.number <= currentStep ? colors.accentBlue : colors.borderLight, width: '100%' }}
                  />
                </div>
              ))}
            </div>
            <div className="text-xs text-right mt-1" style={{ color: colors.textMuted }}>
          ...
          {/* Footer */}
          <div className="border-t p-3 flex justify-between items-center gap-3 flex-shrink-0" style={{ backgroundColor: colors.bgSubtle }}>
            <button
              onClick={handlePrevious}
              disabled={currentStep === 1}
              className="px-4 py-2 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-2 text-sm font-medium"
            ...
            <div className="p-4 md:p-8">
              ...
              <div className="flex gap-3 justify-end">
                <button
                  ...
                  className="px-6 py-2 border rounded-lg transition-colors font-semibold"
