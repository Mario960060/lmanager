...
interface UserGroup {
  user_id: string;
  user_name: string;
  records: MaterialAddedRecord[];
}

interface AdminMaterialAddedModalProps {
  onClose: () => void;
}

const AdminMaterialAddedModal: React.FC<AdminMaterialAddedModalProps> = ({ onClose }) => {
  ...
  // Fetch all materials delivered records
  const { data: userGroups = [], isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Process records to include material name
      const processedRecords = records.map(record => {
        return {
          ...record,
          material_name: record.materials?.name || t('event:unknown_material')
        };
      });
      ...
      processedRecords.forEach(record => {
        ...
        groupedByUser[userId].records.push(record as MaterialAddedRecord);
      });
  ...
  const deleteRecord = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['admin_materials_delivered_records'] });
      ...
      <div className="rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col" style={{ backgroundColor: colors.bgCard }}>
        ...
        <div className="p-4 border-b flex justify-between items-center sticky top-0 z-10" style={{ borderColor: colors.borderDefault, backgroundColor: colors.bgCard }}>
          <h2 className="text-xl font-semibold flex items-center">
            ...
            {t('event:delete_material_added_records_title')}
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="p-4 border-b" style={{ borderColor: colors.borderDefault }}>
          <input
            type="text"
            placeholder={t('event:search_material_added_records')}
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-10">
              ...
              <p className="text-center" style={{ color: colors.red }}>
                {t('event:error_loading_material_added')}
              </p>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-10">
              ...
              <p className="text-center" style={{ color: colors.textSubtle }}>
                {searchTerm ? t('event:no_material_added_records_match') : t('event:no_material_added_records')}
              </p>
            </div>
          ) : (
                          ...
                          <div className="flex justify-between">
                            <div>
                              <h3 className="font-medium" style={{ color: colors.accentBlue }}>
                                {record.material_name || t('event:unnamed_task')}
                              </h3>
                              <div className="mt-1 text-sm">
                                ...
                                <div className="mt-1">
                                  <span className="font-medium">{t('event:amount_label')}</span> {record.amount}
                                </div>
                                {record.notes && (
                                  <div className="mt-1">
                                    <span className="font-medium">{t('event:notes_label')}:</span> {record.notes}
                                  </div>
                                )}
                                {record.project_id && (
                                  <div className="mt-1">
                                    <span className="font-medium">{t('event:project_colon')} ID:</span> {record.project_id}
                                  </div>
                                )}
                              </div>
                            </div>
                            <button
                              onClick={() => setDeleteConfirmation({
                                recordId: record.id,
                                recordName: record.material_name || t('common:this_material')
                              })}
...
export default AdminMaterialAddedModal;
