    ...
    mutationFn: async (recordId: string) => {
      ...
      const { error } = await supabase
        ...
        .insert({
          ...
          record_details: {
            material: record.materials_delivered?.name || 'Unknown Material',
            project: record.events?.title || 'Unknown Project',
            amount: `${record.amount} ${record.materials_delivered?.unit || 'units'}`,
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
          ...
          <div className="p-4 border-b dark:border-gray-700">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search materials by name, project, amount or notes..."
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {isLoading ? (
                    ...
                    <div className="flex justify-between">
                      <div className="flex-1">
                        <h3 className="font-medium text-lg text-gray-900 dark:text-gray-100 mb-2">
                          {delivery.materials_delivered?.name || 'Unknown Material'}
                        </h3>
                        <p className="text-gray-700 dark:text-gray-300 mb-1">
                          Project: <span className="font-medium">{delivery.events?.title || 'Unknown Project'}</span>
                        </p>
                        <p className="text-gray-700 dark:text-gray-300 mb-1">
                          Amount: <span className="font-medium">{delivery.amount} {delivery.materials_delivered?.unit || 'units'}</span>
                        </p>
                        <p className="text-gray-700 dark:text-gray-300 mb-1">
                          Date: <span className="font-medium">{new Date(delivery.delivery_date).toLocaleDateString()}</span>
                        </p>
                        {delivery.notes && (
                        ...
                        <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                          Added on {new Date(delivery.created_at).toLocaleDateString()} at {new Date(delivery.created_at).toLocaleTimeString()}
                        </p>
                      </div>
                      <button
              ...
              <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                {searchTerm ? "No materials match your search." : "You haven't added any material deliveries yet."}
              </div>
            )}
          </div>
        </div>
