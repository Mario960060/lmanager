...
const CheckWeeklyHoursModal = ({ open, onClose }) => {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Check Weekly Hours">
      ...
      <div className="mb-6">
        ...
        {selectedTimeRange === 'preset' && (
          <div className="flex gap-2 mb-4">
            <button className={`px-4 py-2 rounded-lg ${tab === 'today' ? 'bg-gray-700 text-white' : 'bg-gray-100'}`} onClick={() => setTab('today')}>Today</button>
            <button className={`px-4 py-2 rounded-lg ${tab === 'yesterday' ? 'bg-gray-700 text-white' : 'bg-gray-100'}`} onClick={() => setTab('yesterday')}>Yesterday</button>
            <button className={`px-4 py-2 rounded-lg ${tab === 'thisweek' ? 'bg-gray-700 text-white' : 'bg-gray-100'}`} onClick={() => setTab('thisweek')}>This Week</button>
            <button className={`px-4 py-2 rounded-lg ${tab === 'lastweek' ? 'bg-gray-700 text-white' : 'bg-gray-100'}`} onClick={() => setTab('lastweek')}>Last Week</button>
          </div>
        )}
      ...
      <div className="space-y-4">
        ...
        <div className="bg-gray-700 text-white rounded-lg mb-4 px-6 py-4 flex flex-col items-start">
          ...
          <p className="text-3xl font-bold mt-1">{totalHours.toFixed(2)} hours</p>
        </div>
        {Object.entries(grouped).map(([eventId, project]) => (
          <div key={eventId} className="bg-gray-700 text-white rounded-lg">
            <button className="w-full flex items-center justify-between px-6 py-4 focus:outline-none" onClick={() => setExpanded(e => ({ ...e, [eventId]: !e[eventId] }))}>
              <div>
                <h3 className="font-medium">{eventTitles[eventId] || 'Project'}</h3>
                <p className="text-lg font-semibold mt-1">{project.entries.reduce((sum, e) => sum + (e.hours_spent || 0), 0).toFixed(2)} hours</p>
              </div>
                ...
                {(tab === 'today' || tab === 'yesterday' ? 
                  ...
                  project.entries.map((entry, idx) => (
                    <div key={idx} className="flex justify-between text-sm py-1 border-b border-gray-700 last:border-b-0">
                      ...
                      <span>{entry.hours_spent?.toFixed(2)} hours</span>
                    </div>
                  ))
                  : 
                    ...
                    return (
                      <div key={date} className="mb-2">
                        <div className="flex justify-between items-center text-sm font-medium text-gray-400 mb-1">
                          ...
                          <span className="text-gray-300 font-semibold">{dailyTotal.toFixed(2)} hrs</span>
                        </div>
