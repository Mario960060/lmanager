        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
          ...
          <div className="p-4 border-b dark:border-gray-700">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search notes by content, project or date..."
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {isLoading ? (
                    ...
                    <div className="flex justify-between">
                      <div className="flex-1">
                        <h3 className="font-medium text-lg text-gray-900 dark:text-gray-100 mb-2">
                          {eventNames[note.event_id] || 'Unknown Project'}
                        </h3>
                        <p className="text-gray-700 dark:text-gray-300 mb-1">
                          Date: <span className="font-medium">{new Date(note.date).toLocaleDateString()}</span>
                        </p>
                        <div className="mt-3 p-3 bg-gray-50 dark:bg-gray-700 rounded">
                          {note.content || 'No content'}
                        </div>
                        <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                          Added on {new Date(note.created_at).toLocaleDateString()} at {new Date(note.created_at).toLocaleTimeString()}
                        </p>
                      </div>
                      <button
                        onClick={() => {
                          setDeleteConfirmation({
                            isOpen: true,
                            recordId: note.id,
                            recordName: eventNames[note.event_id] || 'Unknown Project'
                          });
                        }}
              ...
              <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                {searchTerm ? "No notes match your search." : "You haven't added any day notes yet."}
              </div>
            )}
          </div>
        </div>
