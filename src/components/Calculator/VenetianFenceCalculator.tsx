...
const VenetianFenceCalculator: React.FC<VenetianFenceCalculatorProps> = ({ 
  ...
  const calculate = async () => {
    ...
    // Calculate labor hours - based on batons/slats count (unit: baton, estimated_hours per baton)
    let mainTaskHours = 0;
    ...
    const layingUnit = layingTask?.unit || 'baton';
  ...
  return (
    <div style={{ fontFamily: fonts.body, display: 'flex', flexDirection: 'column', gap: spacing["6xl"] }}>
      <h2 style={{ fontSize: fontSizes["2xl"], fontWeight: fontWeights.extrabold, color: colors.textPrimary, fontFamily: fonts.display, letterSpacing: '0.3px', margin: `${spacing.md}px 0 ${spacing.sm}px` }}>
        {t('calculator:venetian_fence_calculator_title')}
      </h2>
      ...
      <Card padding={`${spacing["6xl"]}px ${spacing["6xl"]}px ${spacing.md}px`} style={{ marginBottom: spacing["5xl"] }}>
        ...
        <CalculatorInputGrid columns={2}>
          <SelectDropdown
            label={t('calculator:slat_width_cm_label')}
            value={slatWidth + ' cm'}
            options={['4.5 cm', '5 cm']}
            onChange={(v) => setSlatWidth(v.replace(/\s*cm\s*$/, ''))}
            placeholder={t('calculator:slat_width_cm_label')}
          />
          <SelectDropdown
            label={t('calculator:slat_length_cm_label')}
            value={slatLength + ' cm'}
            options={['180 cm', '360 cm']}
            onChange={(v) => setSlatLength(v.replace(/\s*cm\s*$/, ''))}
            placeholder={t('calculator:slat_length_cm_label')}
          />
        </CalculatorInputGrid>

        <TextInput
          label={t('calculator:gaps_between_slats_label')}
          value={gapsBetweenSlats}
          onChange={setGapsBetweenSlats}
