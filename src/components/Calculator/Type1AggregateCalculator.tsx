...
const Type1AggregateCalculator: React.FC<Type1AggregateCalculatorProps> = ({ onResultsChange }) => {
  ...
  const calculateTime = () => {
    ...
    if (!selectedCompactor) {
      alert('Please select a compactor');
      return;
    }
    ...
    if (depthCm <= 0) {
      alert('Please enter valid depth in centimeters');
      return;
    }
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              ...
              <input
                ...
                step="0.01"
              />
              <span className="text-xs text-gray-500">Enter the depth in centimeters (cm)</span>
            </div>
          </div>
        )}
      </div>
