...
const KerbsEdgesAndSetsCalculator: React.FC<CalculatorProps> = ({ 
  ...
  const calculate = async () => {
    ...
    if (isNaN(lengthM) || isNaN(baseHeightCm)) {
      setError('Please enter valid numbers for length and base height');
      return;
    }

    if (lengthM <= 0 || baseHeightCm <= 0) {
      setError('Length and base height must be greater than 0');
      return;
    }
    ...
    try {
      ...
      if (!relevantTask) {
        setError('Could not find task template for labor calculation');
        return;
      }

      const laborHours = relevantTask.estimated_hours * lengthM;

      // Create task breakdown with proper format
      const taskBreakdown: {name?: string; task?: string; hours: number; quantity?: number; amount?: number | string; unit: string; normalizedHours?: number}[] = [{
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {kerbType === 'rumbled' && (
          <div>
            <label className="block text-sm font-medium text-gray-700">Installation Method</label>
            <div className="mt-2 space-x-4">
              ...
              <label className={`inline-flex items-center p-2 rounded text-white ${isRumbledStanding ? 'bg-blue-600' : 'bg-gray-500'}`}>
                <input
                  ...
                  className="form-radio"
                />
                <span className="ml-2">Standing</span>
              </label>
            </div>
          </div>
        )}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Linear Metres</label>
          <input
            type="number"
            value={length}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Base Mortar Height (cm)</label>
          <input
            type="number"
            value={baseHeight}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Hunch Configuration</label>
        ...
        {result && (
          <div className="mt-6 space-y-4" ref={resultsRef}>
            <div>
              <h3 className="text-lg font-medium">Total Labor Hours: <span className="text-blue-600">{result.hours_worked.toFixed(2)} hours</span></h3>
