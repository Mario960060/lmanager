...
const PavingCalculator: React.FC<PavingCalculatorProps> = ({ 
  ...
  const { data: tape1PreparationTasks = [] } = useQuery({
    queryKey: ['tape1_preparation_tasks'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('event_tasks_with_dynamic_estimates')
        .select('*')
  ...
  const { data: soilExcavationTasks = [] } = useQuery({
    queryKey: ['soil_excavation_tasks'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('event_tasks_with_dynamic_estimates')
        .select('*')
    ...
    try {
      ...
      if (calculateDigging && activeExcavator) {
        ...
        if (tape1ThicknessCm && parseFloat(tape1ThicknessCm) > 0) {
          const tape1Tonnes = tape1Volume * 2.1; // Use consistent 2.1 tonnes per cubic meter for tape1
          ...
          if (tape1Template && tape1Template.estimated_hours) {
            // Use estimated_hours as rate per tonne and multiply by actual tonnage
            tape1PreparationTime = tape1Template.estimated_hours * tape1Tonnes;
          } else {
            // Fallback to manual calculation
            const prepExcavationTime = findDiggerTimeEstimate(excavatorSize, tape1Tonnes);
            const prepTransportTime = selectedCarrier ? findCarrierTimeEstimate(carrierSize, tape1Tonnes) : 0;
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <input
          ...
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-gray-600 focus:ring-gray-600"
          placeholder="Enter additional soil depth in cm"
          min="0"
          step="0.5"
        />
