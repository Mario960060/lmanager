  ...
  const inviteUserMutation = useMutation({
    mutationFn: async ({ email, role }: { email: string; role: string }) => {
      ...
      try {
        ...
        if (profileError) throw profileError;
        if (!user) throw new Error('User must have an account first. They can create one by visiting the app.');

        console.log('✅ Found user ID:', user.id);

        // If user already belongs to another company
        if (user.company_id) {
          throw new Error('This user already belongs to another company. They must leave that company first.');
        }
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['company_members'] });
      setUpdateSuccess(`User ${data.email} has been invited successfully!`);
      setUpdateError(null);
      setInviteEmail('');
      setInviteRole('user');
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-800 rounded-lg max-w-2xl w-full max-h-[90vh] flex flex-col">
        <div className="flex justify-between items-center p-6 border-b dark:border-slate-700 flex-none">
          <h2 className="text-xl font-semibold dark:text-white">User Authorization</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto">
          ...
          <div className="space-y-4">
            {isLoading ? (
              <p className="text-center py-4 dark:text-gray-400">Loading users...</p>
            ) : members.length === 0 ? (
              <p className="text-center py-4 dark:text-gray-400">No users found</p>
            ) : (
                ...
                <div key={member.id} className="bg-gray-50 dark:bg-slate-700 p-4 rounded-lg">
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400">Current Role: <span className="font-medium text-gray-700 dark:text-gray-300">{member.role}</span></p>
                    </div>
                    <div>
                      <select
                        value={member.role}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-[51] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-lg max-w-md w-full">
            <div className="flex justify-between items-center p-6 border-b dark:border-slate-700">
              <h3 className="text-lg font-semibold dark:text-white">Add User to Company</h3>
              <button
            ...
            <div className="p-6 space-y-4">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">
                Note: The user must have an account already. If they don't, they can create one from the registration page.
              </p>
